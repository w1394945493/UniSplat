...
template<int K>
__device__ void updateKBest(const float3& ref, const float3& point, int pointIdx, float* knn_dist, int* knn_idx)
{
...
__global__ void boxMeanDist(uint32_t P, float3* points, uint32_t* indices, MinMax* boxes, float* dists, int* knn_indices)
{
	...
	for (int i = max(0, idx - 3); i <= min(P - 1, idx + 3); i++)
	{
		if (i == idx)
			continue;
		updateKBest<3>(point, points[indices[i]], indices[i], best, best_idx);
	}
	...
	knn_indices[original_idx * 3] = best_idx[0];
    knn_indices[original_idx * 3 + 1] = best_idx[1];
    knn_indices[original_idx * 3 + 2] = best_idx[2];
}

void SimpleKNN::knn(int P, float3* points, float* meanDists, int* knnIndices)
{
	float3* result;
	...
	boxMeanDist<<<num_boxes, BOX_SIZE>>>(P, points, indices_sorted.data().get(), boxes.data().get(), meanDists, knnIndices);

	cudaFree(result);
}

__global__ void boxNearestCrossSetDist(uint32_t P, float3* points, uint32_t* indices, 
                                       MinMax* boxes, int* labels, float* nearestDists, int* nearestIndices)
{
    int idx = cg::this_grid().thread_rank();
    if (idx >= P)
        return;
    ...
    int queryLabel = labels[originalIdx];
    
    if (queryLabel != 0) {
        nearestDists[originalIdx] = FLT_MAX;
		nearestIndices[originalIdx] = -1;
        return;
    }
    
    float3 point = points[originalIdx];
    ...
    for (int i = max(0, idx - 3); i <= min(P - 1, idx + 3); i++)
    {
        int candidateOriginalIdx = indices[i];
        if (labels[candidateOriginalIdx] == 1)
        {
            float3 candidate = points[candidateOriginalIdx];
            float3 d = { candidate.x - point.x, candidate.y - point.y, candidate.z - point.z };
            float dist = d.x * d.x + d.y * d.y + d.z * d.z;
            if (dist < bestDist) {
                bestDist = dist;
                bestIdx = candidateOriginalIdx;
            }
        }
    }
    ...
    {
        MinMax box = boxes[b];
        float boxDist = distBoxPoint(box, point);
        if (boxDist >= bestDist)
            continue;
        for (int i = b * BOX_SIZE; i < min(P, (b + 1) * BOX_SIZE); i++)
        {
    ...
    nearestDists[originalIdx] = sqrt(bestDist);
	nearestIndices[originalIdx] = bestIdx;
}

void SimpleKNN::nearestCrossSetDistance(int P, float3* points, int* labels, float* nearestDists, int* nearestIndices)
{
    float3* result;
    cudaMalloc(&result, sizeof(float3));
    size_t temp_storage_bytes;
    ...
    boxNearestCrossSetDist<<<num_boxes, BOX_SIZE>>>(P, points, indices_sorted.data().get(), 
                                                     boxes.data().get(), labels, nearestDists, nearestIndices);
