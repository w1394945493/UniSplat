...
def SeqColorJitter():
    ...
    gamma = _check_input(0.3, center=1.0)
    apply_jpeg = random.random() < 0.5
    jpeg_quality = random.randint(20, 100) if apply_jpeg else None
    ...
    gamma_factor = (
        None
        if gamma is None
        else float(torch.empty(1).uniform_(gamma[0], gamma[1]))
    )
    ...
    def _color_jitter(img):
        for fn_id in fn_idx:
            ...
            elif fn_id == 4 and gamma_factor is not None:
        ...
        if apply_jpeg:
            ...
            # 应用JPEG压缩
            _, encoded_img = cv2.imencode('.jpg', img_np_bgr, [cv2.IMWRITE_JPEG_QUALITY, jpeg_quality])
            img_np_bgr = cv2.imdecode(encoded_img, cv2.IMREAD_COLOR)
